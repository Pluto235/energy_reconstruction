...
<main>
  ...
  <section>
    ...
    {figure(active_psf_norm_profiles_path or Path('__missing_active_psf_norm_profiles.png'), 'Active 30-cell own-cell normalized radial profiles', wide=True, explanation='Diagnostic-only view of each selected cell own radial MC distribution, normalized by that cell peak. Colored solid curves are the cell own MC radial distributions; gray vertical dashed lines mark the fit PSF sigma_deg position for that cell. The only active borrowed-PSF cells are 39, 52, and 65: the fit uses 39 -> 40, 52 -> 2/3*53 + 1/3*54, and 65 -> 2/3*66 + 1/3*67. Cell 69 is direct and does not borrow a PSF.')}
