...
APERTURE_G_META = (
    REPO_ROOT
    / "apply/output/stage_g_v4_aperture_conditioned/runs/v4_stage_g_aperture_conditioned_drop4/sed_points_v4_aperture_conditioned_drop4_metadata.json"
)
DROP4_G_META = (
    REPO_ROOT
...
def expected_raw_counts(response: dict[str, np.ndarray], signal: dict[str, np.ndarray], theta_exposure: np.ndarray) -> np.ndarray:
    ...
    signal_cell_id = np.asarray(signal["cell_id"], dtype=np.int64)
    response_index = {int(cid): idx for idx, cid in enumerate(response_cell_id)}
    missing = [int(cid) for cid in signal_cell_id if int(cid) not in response_index]
    if missing:
        raise ValueError(f"Response is missing signal cells: {missing}")
    index = np.asarray([response_index[int(cid)] for cid in signal_cell_id], dtype=np.int64)
...
def expected_modes(
    ...
    theta_exposure: np.ndarray,
) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    all_direction_raw = expected_raw_counts(response, signal, theta_exposure)
    containment = np.asarray(signal["containment_r_opt"], dtype=np.float64)
    modes: dict[str, np.ndarray] = {
        "nominal_containment": all_direction_raw * containment,
        "containment_1": all_direction_raw,
    }
    meta: dict[str, Any] = {
        "aperture_conditioned_available": False,
        "all_direction_response_npz": str(RESPONSE_NPZ),
        "aperture_conditioned_response_npz": str(APERTURE_RESPONSE_NPZ),
    }
    if APERTURE_RESPONSE_NPZ.exists():
        aperture_response = load_npz(APERTURE_RESPONSE_NPZ)
        modes["aperture_response_containment_1"] = expected_raw_counts(aperture_response, signal, theta_exposure)
        meta["aperture_conditioned_available"] = True
    return modes, meta


def expected_reference_factor(expected: np.ndarray, reference: np.ndarray, mask: np.ndarray) -> float | str:
    ref_sum = float(np.nansum(reference[mask]))
    if ref_sum <= 0.0:
        return ""
    return float(np.nansum(expected[mask]) / ref_sum)


def mode_title(mode: str) -> str:
    return {
        "nominal_containment": "All-direction response x containment_r_opt",
        "containment_1": "All-direction response x 1",
        "aperture_response_containment_1": "Aperture-conditioned response x 1",
    }.get(mode, mode)


def mode_short_label(mode: str) -> str:
    return {
        "nominal_containment": "current",
        "containment_1": "all-dir c=1",
        "aperture_response_containment_1": "aperture response c=1",
    }.get(mode, mode)


def selector_summary_rows(
    signal: dict[str, np.ndarray],
    expected_by_mode: dict[str, np.ndarray],
) -> list[dict[str, Any]]:
    ...
    reference = expected_by_mode["containment_1"]
    for selector, mask in selectors.items():
        for containment_mode, expected in expected_by_mode.items():
            ...
            rows.append(
                {
                    ...
                    "expected_over_all_direction_c1": expected_reference_factor(expected, reference, mask),
                }
            )
                ...
                rows.append(
                    {
                        ...
                        "observed_over_expected": total_excess / total_expected if total_expected > 0 else "",
                        "N_on_over_B_on": total_n / total_b if total_b > 0 else "",
                        "expected_over_all_direction_c1": expected_reference_factor(expected, reference, nhit_mask),
                    }
                )
    return rows
...
def cell_rows(
    ...
    for idx, cid in enumerate(cell_id):
        ...
        nominal = float(expected_by_mode["nominal_containment"][idx])
        containment1 = float(expected_by_mode["containment_1"][idx])
        aperture = (
            float(expected_by_mode["aperture_response_containment_1"][idx])
            if "aperture_response_containment_1" in expected_by_mode
            else float("nan")
        )
        rows.append(
            {
                ...
                "official_expected_aperture_response": aperture if math.isfinite(aperture) else "",
                "ratio_nominal": excess / nominal if nominal > 0 else "",
                "ratio_containment1": excess / containment1 if containment1 > 0 else "",
                "ratio_aperture_response": excess / aperture if aperture > 0 else "",
                ...
                "required_delta_b_over_b_aperture_response": (excess - aperture) / b_on if b_on > 0 and aperture > 0 else "",
            }
        )
...
def stage_f_comparison_rows() -> list[dict[str, Any]]:
    ...
    for label, path in [
        ...
        ("v4_drop4_aperture_response_containment_1", APERTURE_F_META),
    ]:
        meta = load_json(path)
        if not meta:
            continue
...
def plot_selector_ratios(rows: list[dict[str, Any]], path: Path) -> None:
    ...
    modes = [
        mode
        for mode in ["nominal_containment", "containment_1", "aperture_response_containment_1"]
        if any(row.get("containment_mode") == mode for row in rows)
    ]
    labels = sorted(
        {
            str(row["nhit_bin"])
    ...
    fig, axes = plt.subplots(1, len(modes), figsize=(6.2 * len(modes), 4.8), dpi=160, sharey=True)
    ...
    for ax, mode in zip(axes_list, modes):
        ...
        ax.grid(alpha=0.25)
    axes_list[0].set_ylabel("Stage E excess / official pass5 folded counts")
    axes_list[-1].legend(fontsize=8)
    fig.suptitle("Response/aperture contract changes low-Nhit official forward-fold ratios")
    fig.tight_layout()
    fig.savefig(path)
    plt.close(fig)
...
def plot_sed_comparison(path: Path) -> None:
    ...
    for meta_path, color, marker, label_prefix in [
        ...
        (APERTURE_G_META, "#059669", "^", "v4 drop4 aperture response, c=1"),
    ]:
    ...
    ax.set_title("SED response/aperture contract comparison")
...
def main() -> None:
    ...
    expected_by_mode, mode_meta = expected_modes(response, signal, theta_exposure)

    summary_rows = selector_summary_rows(signal, expected_by_mode)
    write_csv(
        ...
        [
            ...
            "N_on_over_B_on",
            "expected_over_all_direction_c1",
        ],
    )

    per_cell = cell_rows(signal, expected_by_mode)
    write_csv(
    ...
    drop4_aperture_all = next(
        (
            row
            for row in summary_rows
            if row["selector"] == "drop4"
            and row["containment_mode"] == "aperture_response_containment_1"
            and row["nhit_bin"] == "all"
        ),
        {},
    )
    response_meta = load_json(RESPONSE_META)
    aperture_response_meta = load_json(APERTURE_RESPONSE_META)
    aperture_ratio = drop4_aperture_all.get("observed_over_expected", "")
    interpretation = "pending_aperture_conditioned_stage_a"
    aperture_expected_over_nominal = ""
    nominal_expected_over_aperture = ""
    if aperture_ratio != "":
        nominal_expected = float(drop4_nominal_all["official_expected_counts"])
        aperture_expected = float(drop4_aperture_all["official_expected_counts"])
        aperture_expected_over_nominal = aperture_expected / nominal_expected if nominal_expected > 0 else float("nan")
        nominal_expected_over_aperture = nominal_expected / aperture_expected if aperture_expected > 0 else float("nan")
        interpretation = "scalar_containment_suppresses_expectation_beyond_aperture_conditioned_response"
    write_json(
        OUT_DIR / "v4_response_audit_summary.json",
        {
            "summary": {
                "dominant_current_hypothesis": (
                    "Default Stage A is an all-direction cell response and Stage F/G multiply by containment_r_opt to predict "
                    "on-aperture source counts. The definitive contract check is the new aperture-conditioned Stage A response "
                    "used downstream with containment_r_opt=1."
                ),
                "double_containment_interpretation": interpretation,
                "aperture_conditioned_available": bool(mode_meta.get("aperture_conditioned_available")),
                ...
                "drop4_aperture_conditioned_official_obs_over_expected": aperture_ratio,
                "drop4_nominal_expected_over_all_direction_c1": drop4_nominal_all["expected_over_all_direction_c1"],
                "drop4_aperture_expected_over_all_direction_c1": drop4_aperture_all.get("expected_over_all_direction_c1", ""),
                "drop4_aperture_expected_over_nominal": aperture_expected_over_nominal,
                "drop4_nominal_expected_over_aperture": nominal_expected_over_aperture,
                ...
                "stage_a_response_aperture_conditioning": response_meta.get("response_aperture_conditioning"),
                "stage_a_metadata_mc_dangle_cut": response_meta.get("cuts", {}).get("mc_dangle_cut")
                if isinstance(response_meta.get("cuts"), dict)
                else None,
                "aperture_stage_a_response_type": aperture_response_meta.get("response_type"),
                "aperture_stage_a_response_aperture_conditioning": aperture_response_meta.get("response_aperture_conditioning"),
                "caveat": (
                    "The response-contract comparison should be read from expected counts, not only obs/expected ratios. "
                    "Aperture-conditioned response x 1 is the closest direct definition of on-aperture effective area. "
                    "If the current all-direction x scalar-containment expectation is below that branch, the scalar containment "
                    "is over-suppressing expected counts relative to the aperture-conditioned response."
                ),
            },
            "outputs": {
