...
opt0_job=$(sbatch --parsable apply/scripts/slurm/optimize_v6_empirical_cdf_asimov_iter0.sbatch)
...
final_job=$(sbatch --parsable --dependency=afterok:"${iter0_job}" \
  --export=ALL,ANALYTIC_BON_BASELINE_SHA="${ANALYTIC_BON_BASELINE_SHA}",OPT0_JOB_ID="${opt0_job}",ITER0_JOB_ID="${iter0_job}" \
