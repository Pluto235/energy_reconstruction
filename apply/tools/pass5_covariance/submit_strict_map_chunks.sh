...
cd "$RUN_DIR"
hep_sub -g lhaaso -mem "$MEMORY_MB" -prio "$PRIORITY" \
    "$RUN_DIR/run_strict_map_chunk.sh" -argu "%{ProcId}" -n "$job_count"

echo "STRICT_MAP_ARRAY_SUBMITTED jobs=$job_count memory_mb=$MEMORY_MB priority=$PRIORITY"
