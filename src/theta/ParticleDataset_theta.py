...
class ParticleDataset(Dataset):
    ...
    New optional args for Slurm param sweep:
      ...
      - core_scale: (sx, sy) used to normalize reconstructed core (xc, yc)
    ...
    def _load_file(
        ...
        try:
            ...
            for i in range(n_events):
                ...
                # Keep reconstructed core as a separate event-level condition.
                reco_core_xy = np.array(
                    [
                        float(arrays["xc"][i]) / float(core_scale[0]),
                        float(arrays["yc"][i]) / float(core_scale[1]),
                    ],
                    dtype=np.float32,
                )
    ...
    def __getitem__(self, idx: int):
        ...
        reco_core_xy = proc["reco_core_xy"]
        ...
        reco_core_t = torch.tensor(reco_core_xy, dtype=torch.float32)  # (2,)

        return points_t, features_t, mask_t, costheta_t, reco_core_t, log_energy_t, mc_weight_t
