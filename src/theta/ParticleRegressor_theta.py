...
class ParticleNetRegressor(nn.Module):
    ...
    def __init__(self, input_dims: int,
        ...
        # Event-level reconstructed-core embedding using normalized (xc, yc).
        if core_embed_dim > 0:
    ...
    def forward(self,
                ...
                reco_core_xy: torch.Tensor = None
                ):
        """
        前向传播
        输入:
        ...
        if self.core_mlp is not None:
            if reco_core_xy is None:
                raise ValueError("core_embed_dim>0 but reco_core_xy is None. Please pass reco_core_xy to forward().")
            if reco_core_xy.dim() == 1:
                reco_core_xy = reco_core_xy.unsqueeze(0)
            reco_core_xy = reco_core_xy.reshape(reco_core_xy.shape[0], -1)
            if reco_core_xy.shape[1] != 2:
                raise ValueError(f"Expected reco_core_xy with shape (B,2), got {tuple(reco_core_xy.shape)}")

            cond_embeddings.append(self.core_mlp(reco_core_xy.float()))
