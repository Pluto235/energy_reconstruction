...
def evaluate_model(
    ...
    preds, trues, ws = [], [], []
    costheta_all, reco_core_all = [], []

    def _unpack_batch(batch):
        if len(batch) == 7:
            points, features, mask, costheta, reco_core_xy, logE_true, mc_weight = batch
        elif len(batch) == 6:
        ...
        return points, features, mask, costheta, reco_core_xy, logE_true, mc_weight

    with torch.no_grad():
        for batch in test_loader:
            points, features, mask, costheta, reco_core_xy, logE_true, mc_weight = _unpack_batch(batch)

            points = points.to(device, non_blocking=True)
            features = features.to(device, non_blocking=True)
            mask = mask.to(device, non_blocking=True)
            costheta = costheta.to(device, non_blocking=True) if costheta is not None else None
            reco_core_xy = reco_core_xy.to(device, non_blocking=True) if reco_core_xy is not None else None
            logE_true = logE_true.to(device, non_blocking=True)

            logE_pred = model(points, features, mask, costheta, reco_core_xy)
            ...
            if reco_core_xy is not None:
                reco_core_all.append(reco_core_xy.detach().cpu().numpy())
    ...
    reco_core_np = np.concatenate(reco_core_all, axis=0).astype(np.float32) if reco_core_all else np.empty((0, 2), dtype=np.float32)
    ...
    if reco_core_np.size > 0:
        reco_core_np = reco_core_np[valid]
    ...
    if out_dir is not None:
        ...
        if save_arrays:
            payload = dict(
                ...
                reco_core_xy=to_numpy(reco_core_np),
