...
def build_argparser():
    ...
    p.add_argument("--core_embed_dim", type=int, default=16, help="embedding dim for normalized reconstructed core (0 disables)")
    p.add_argument("--core_embed_dropout", type=float, default=0.0, help="dropout in reconstructed-core embedding MLP")
    p.add_argument("--core_scale_x", type=float, default=130.0, help="normalization scale for xc")
    p.add_argument("--core_scale_y", type=float, default=110.0, help="normalization scale for yc")
...
def main(args):
    ...
    with torch.no_grad():
        ...
        if len(batch) == 7:
            test_points, test_features, test_mask, test_costheta, test_reco_core, test_energies, _w = batch
        elif len(batch) == 6:
        ...
        test_reco_core = test_reco_core.to(device, non_blocking=True) if test_reco_core is not None else None

        out = model(test_points, test_features, test_mask, test_costheta, test_reco_core)
